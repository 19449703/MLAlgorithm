...
class DecisionTree:
    # 定义节点类 二叉树
    class Node:
        def __init__(self, parent = None, label = None, feature_name = None):
            self.parent = parent
            self.label = label
    ...
    def train(self, train_data, parent_node = None):
        ...
        max_feature_col, max_feature_rule_value = DecisionTree.choose_best_feature(np.array(train_data), self.type)
        ...
        for v in max_feature_value_list:
            ...
            sub_node = self.train(sub_train_df, parent_node)
